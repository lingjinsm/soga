...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://raw.githubusercontent.com/lingjinsm/soga/main/latest.json" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://raw.githubusercontent.com/lingjinsm/soga/main/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://raw.githubusercontent.com/lingjinsm/soga/main/soga-cracked-linux64.tar.gz"
    ...
    curl -o /usr/bin/soga -Ls https://raw.githubusercontent.com/lingjinsm/soga/main/soga.sh
